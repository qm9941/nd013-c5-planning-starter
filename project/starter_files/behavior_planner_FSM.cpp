...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //Look ahead distance:
  //Distance to come to a stop when while traveling at speed V and
  // using a resonable deceleration of 1 m/s^2
  const double a = 1; //abs deceleration [m/s^2}
  double look_ahead_distance = std::pow(velocity_mag, 2) / (2 * a);
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) 
  {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += std::cos(ang) * _stop_line_buffer;
      goal.location.y += std::sin(ang) * _stop_line_buffer;
  ...
  else if (_active_maneuver == DECEL_TO_STOP) 
  {
    ...
    // new goal is the same as the previous goal (_goal). 
    goal = _goal;
