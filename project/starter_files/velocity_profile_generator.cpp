...
double VelocityProfileGenerator::calc_distance(const double& v_i,
  ...
  if (std::abs(a) < DBL_EPSILON) {
    ...
    // compute the distance required for a given constant acceleration/deceleration.
...
double VelocityProfileGenerator::calc_final_speed(const double& v_i,
  ...
  double v_f{0.0};
  // compute the final speed for a given constant acceleration across a given distance, with initial speed v_i.
